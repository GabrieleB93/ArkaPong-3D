class Pad{
    ...
    PcpadMove(difficulty){
      ...
      //  let paddleRightCx = this.pad.position.x;
        //let dx = Math.min(difficulty, Math.abs(ball.getPositionX() - paddleRightCx));
        //paddleRightCx += ball.getPositionX() > paddleRightCx ? dx : -dx;

        //let tmpx =(Math.max(-w +wpad*2+13, Math.min(w-wpad*2-13, paddleRightCx))) ;

        //let paddleRightCy = this.pad.position.y;
        //let dy = Math.min(difficulty, Math.abs(ball.getPositionY() - paddleRightCy));
        //paddleRightCy += ball.getPositionY() > paddleRightCy ? dy : -dy;
        //let tmpy =(Math.max(-h+hpad*2+28, Math.min(h-hpad*2-28, paddleRightCy)));
        ...
        ùlet tmpy =(Math.max(-h+hpad*2+28, Math.min(h-hpad*2-28, pady)));
